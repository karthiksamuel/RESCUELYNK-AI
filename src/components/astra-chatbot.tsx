        ...
        <div className="flex flex-col w-full gap-6 animate-fade-in relative z-10 min-h-screen overflow-visible">
            ...
            {messages.length <= 1 && (
                <div className="bg-[#0b0b13] border border-white/5 rounded-xl p-6 shadow-lg">
                    <h2 className="text-lg font-semibold text-white">
                        ASTRA Survival Intelligence (Local LLM)
                    </h2>
                    <p className="text-gray-400 mt-2 text-sm leading-relaxed">
                        Your offline AI survival assistant powered by local retrieval-augmented generation and Ollama.
                    </p>
                    <ul className="mt-4 text-sm text-gray-300 space-y-2 font-mono uppercase tracking-tight">
                        <li className="flex items-center gap-2">
                            <span className="w-1.5 h-1.5 rounded-full bg-info" />
                            Earthquake, tornado, hurricane survival
                        </li>
                        <li className="flex items-center gap-2">
                            <span className="w-1.5 h-1.5 rounded-full bg-info" />
            ...
            {messages.length <= 1 && (
                <div className="w-full space-y-3">
                    <p className="text-[10px] text-info uppercase font-mono tracking-widest font-bold flex items-center gap-2">
                        <Zap className="w-3 h-3" /> Neural Patterns
                    </p>
                    <div className="flex flex-wrap gap-2">
                        {SUGGESTIONS.map(s => (
                            <button key={s.label} onClick={() => send(s.query)}
                                className="text-[10px] font-mono tracking-wide bg-secondary/5 border border-secondary/10 rounded px-3 py-2 text-secondary hover:bg-secondary/15 hover:border-secondary/30 transition-all whitespace-nowrap">
                                {s.label}
                            </button>
                        ))}
                    </div>
